...
function setnextmarkerposition(llp::LineLabelPositioner, i, ax::AxisMap, databox)
  ...
  non_nothing(x) = [a for a in x if !isnothing(a)]
  too_close_to_another_marker(p) = any(a -> norm(p - a) < 2 * llp.separation,
                                       non_nothing(values(llp.markerpositions)))
  ...
  nearly_outside = [close_true(points_outside, i, llp.steps_away_from_outside_axes)
                    for i = 1:length(points_outside)]

  # list of points
  good_points = [all_points_on_line[i]
                 for i = 1:length(all_points_on_line) if !nearly_outside[i]]
...
function setmarkerpositions(llp::LineLabelPositioner, ax::AxisMap, databox)
  for i = 1:(llp.n)
    if isnothing(llp.xmin[i])
      llp.markerpositions[i] = nothing
    else
...
function LineLabelPositioner(ad::AxisDrawable, pll::Vector{PointList}, xdes; kw...)
  ...
  firstx(p) = isempty(p) ? nothing : p[1].x
  lastx(p) = isempty(p) ? nothing : p[end].x
  xmin = [firstx(a.points) for a in pll]
  xmax = [lastx(a.points) for a in pll]
